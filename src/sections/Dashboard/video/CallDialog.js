...
const CallDialog = ({ open, handleClose }) => {
  ...
  // const appID = 1966111766;
  // const server = "wss://webliveroom1966111766-api-bak.coolzcloud.com/ws";
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      socket.emit(
        ...
        { to: streamID, from: userID, roomID },
        () => {
    ...
    <>
      ...
      >
        <DialogContent>
          ...
          <div className=" font-bold text-xl text-center">{callStatus}</div>
        </DialogContent>
        <DialogActions>
          <Button
            disabled={callStatus == "Busy" || callStatus == "Not Picked"}
            onClick={() => {
